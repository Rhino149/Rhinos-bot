...
if (!message.member.permissions.has("ADMINISTRATOR")) {
  return message.reply("You dont have the right perms....").then(m => m.delete(5000));
	...
	if (role.position > author.roles.highest.position) return message.channel.send(`Sorry but the role you wanted to add (${role.name}) position(${role.position}) is higher then your highest roles position(${author.highestRole.position})`)
        try {
  ...
  // Check if a member has a specific permission on the guild     
    ...
    // End the command if the bot cannot find the role on the server.
    if (!role) return message.reply('I can\'t seem to find that role.');
if (role.position > author.roles.highest.position) return message.channel.send(`Sorry but the role you wanted to remove (${role.name}) position(${role.position}) is above  your top role's position(${author.highestRole.position})`)
    try {
