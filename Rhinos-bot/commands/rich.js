...
.filter(u => list.members.cache.has(u.member))
...
lb.map((user, position) => embed.addField("[#**" + (position + 1) + "**] " + client.users.cache.get(user.member).tag, ` They have $${client.money.get(user.member, 'money').toLocaleString()} <:rhino_coin:734247475207995423>`)
  )
